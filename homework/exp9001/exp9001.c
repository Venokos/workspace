...
StackResult stackPush(Stack* stack, char bracket, int position){
    ...
    if (stack->top + 1 == stack->capacity) {
        if (stack->capacity + 1 > stack->max_capacity) {
            return STACK_FULL;
        }
        ...
        if (new_data == NULL) return STACK_MEM_ERR;
...
int jsonBracketCheckBasic(const char *json_str) {
    ...
    Stack tempStack;
    if (json_str == NULL || strlen(json_str) == 0) return 1;
    if (stackInit(&tempStack, 10, 100) != STACK_OK) return 0;
    for (int i = 0; i < strlen(json_str); i++) {
        if (isLeftBracket(json_str[i])) {
            if (stackPush(&tempStack, json_str[i], i) != STACK_OK) return 0;
        }
        else if (isRightBracket(json_str[i])) {
            StackElem temp;
            if (stackPop(&tempStack, &temp) != STACK_OK) return 0;
            if (!isBracketMatch(temp.bracket, json_str[i])) return 0;
        }
    }
    if (stackIsEmpty(&tempStack)) {
        stackDestroy(&tempStack);
        return 1;
    }
...
// 步骤6：销毁栈（stackDestroy）
    ...
    for (int i = 0; i < strlen(json_str); i++) {
        ...
        if (in_string) {
            ...
            else if (ch == '"' && !escape) in_string = 0;
        }
        else {
            if (ch == '"') in_string = 1;
            else if (isLeftBracket(ch)) {
                if (stackPush(&tempStack, ch, i) != STACK_OK) return 0;
            }
            else if (isRightBracket(ch)) {
                StackElem temp;
                if (stackPop(&tempStack, &temp) != STACK_OK) return 0;
                if (!isBracketMatch(temp.bracket, ch)) return 0;
            }
        }
    }
    if (stackIsEmpty(&tempStack) && !in_string) {
        stackDestroy(&tempStack);
        return 1;
    }
    else {
