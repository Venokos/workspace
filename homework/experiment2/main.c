...
float uaFindInterestById(const Customer customers[], int n, int id, Metrics *m) {
    ...
    for (int i = 0; i < n; i++) {
        if (m) m->compares++;
        if (customers[i].id == id) return customers[i].interest;
    }
...
float oaFindInterestById(const Customer customers[], int n, int id, Metrics *m) {
    ...
    int lo = 0, hi = n-1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2; // 循环中才使用的变量mid，不需要在外部声明
        if (m) m->compares++;
...
int oaDeleteById(Customer customers[], int n, int id, Metrics *m) {
    ...
    while (lo <= hi) {
        ...
        if (customers[mid].id == id) {
            idx = mid;
            // 找到后直接跳出循环！这里如果不跳出循环，找到目标值后，mid的值不变(customers[mid].id == id)始终为真，死循环
            break;
        }
        else if (customers[mid].id < id) lo = mid + 1;
        else hi = mid - 1;
...
int main(void) {
    ...
    Metrics m_ua = {0, 0}, m_oa = {0, 0}; // 计数指针
    ...
    while (scanf("%31s", cmd) == 1) {
        if (strcmp(cmd, "UA_INSERT") == 0) {
            ...
            int new_n = uaInsertBack(ua, ua_n, CAPACITY, c, &m_ua);
            if (new_n == -1) {
            ...
            float v = uaFindInterestById(ua, ua_n, id, &m_ua);
            ...
            int id;
            if (scanf("%d", &id) != 1) return 0;
            int new_n = uaDeleteById(ua, ua_n, id, &m_ua);
            if (new_n == -1) {
            ...
            Customer c = { id, interest };
            int new_n = oaInsertKeepOrder(oa, oa_n, CAPACITY, c, &m_oa);
            if (new_n == -1) {
            ...
            int id;
            if (scanf("%d", &id) != 1) return 0;
            float v = oaFindInterestById(oa, oa_n, id, &m_oa);
            if (v == INTEREST_NOT_FOUND) {
            ...
            int id;
            if (scanf("%d", &id) != 1) return 0;
            int new_n = oaDeleteById(oa, oa_n, id, &m_oa);
            if (new_n == -1) {
    ...
    printf("UA_TOTAL: Compares: %lld, Moves: %lld\n", m_ua.compares, m_ua.moves);
    printf("OA_TOTAL: Compares: %lld, Moves: %lld\n", m_oa.compares, m_oa.moves);
